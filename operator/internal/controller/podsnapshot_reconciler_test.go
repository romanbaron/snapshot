...
func makeSnapshotForReconcile() *snapshotv1alpha1.PodSnapshot {
...
func TestSnapshotReconciler_ReadySnapReMirrorsViaBoundContent(t *testing.T) {
	...
	content := &snapshotv1alpha1.PodSnapshotContent{
		...
		Status: snapshotv1alpha1.PodSnapshotContentStatus{
...
func TestSnapshotReconciler_AlreadyFailedShortCircuits(t *testing.T) {
	...
	updated := &snapshotv1alpha1.PodSnapshot{}
	require.NoError(t, r.Get(context.Background(), types.NamespacedName{Namespace: "inference", Name: snap.Name}, updated))
	assert.True(t, meta.IsStatusConditionTrue(updated.Status.Conditions, snapshotv1alpha1.PodSnapshotConditionFailed))
	assert.False(t, meta.IsStatusConditionTrue(updated.Status.Conditions, snapshotv1alpha1.PodSnapshotConditionReady))
}
...
func TestSnapshotReconciler_ContentConflictFails(t *testing.T) {
	...
	content := &snapshotv1alpha1.PodSnapshotContent{
		ObjectMeta: metav1.ObjectMeta{Name: "podsnapshotcontent-snap-uid"},
		Spec: snapshotv1alpha1.PodSnapshotContentSpec{
			PodSnapshotRef: snapshotv1alpha1.PodSnapshotReference{Namespace: "inference", Name: snap.Name, UID: "other-snap-uid"},
			Source:         snapshotv1alpha1.PodSnapshotContentSource{PodRef: snapshotv1alpha1.PodReference{Name: "worker-0", UID: "pod-uid-9"}, NodeName: "node-a"},
		},
	}
	r := makeSnapshotReconciler(s, snap, content, scheduledPod("abc123"))
...
func TestSnapshotReconciler_AdoptsExistingContentAndMirrors(t *testing.T) {
	...
	reconcileSnapshot(t, r, snap.Name)
	bound := &snapshotv1alpha1.PodSnapshot{}
	...
	assert.Nil(t, meta.FindStatusCondition(bound.Status.Conditions, snapshotv1alpha1.PodSnapshotConditionFailed))
...
func TestSnapshotReconciler_MirrorsReadyAndFailed(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "ready", condType: snapshotv1alpha1.PodSnapshotConditionReady},
		{name: "failed", condType: snapshotv1alpha1.PodSnapshotConditionFailed},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			opposite := snapshotv1alpha1.PodSnapshotConditionFailed
			if tc.condType == snapshotv1alpha1.PodSnapshotConditionFailed {
				opposite = snapshotv1alpha1.PodSnapshotConditionReady
			}
...
func TestSnapshotContentToSnapshot_UnwrapsTombstone(t *testing.T) {
	content := &snapshotv1alpha1.PodSnapshotContent{
		...
		Spec: snapshotv1alpha1.PodSnapshotContentSpec{
			PodSnapshotRef: snapshotv1alpha1.PodSnapshotReference{Namespace: "inference", Name: "podsnapshot-abc123"},
		},
	}
	...
	emptyRef := &snapshotv1alpha1.PodSnapshotContent{ObjectMeta: metav1.ObjectMeta{Name: "orphan"}}
...
func TestSnapshotReconciler_EnsureFinalizerAddsThenProceeds(t *testing.T) {
	...
	withFinalizer := &snapshotv1alpha1.PodSnapshot{}
	...
	assert.Nil(t, meta.FindStatusCondition(withFinalizer.Status.Conditions, snapshotv1alpha1.PodSnapshotConditionFailed))
	var contents snapshotv1alpha1.PodSnapshotContentList
	require.NoError(t, r.List(context.Background(), &contents))
	assert.Empty(t, contents.Items)
...
func TestSnapshotReconciler_DeleteWithNoFinalizerIsNoop(t *testing.T) {
	...
	content := &snapshotv1alpha1.PodSnapshotContent{
		ObjectMeta: metav1.ObjectMeta{Name: "podsnapshotcontent-snap-uid"},
		Spec: snapshotv1alpha1.PodSnapshotContentSpec{
			PodSnapshotRef: snapshotv1alpha1.PodSnapshotReference{Namespace: "inference", Name: "podsnapshot-abc123", UID: "snap-uid"},
		},
	}
	...
	assert.NoError(t, r.Get(context.Background(), types.NamespacedName{Name: "podsnapshotcontent-snap-uid"}, &snapshotv1alpha1.PodSnapshotContent{}))
}
...
func TestSnapshotReconciler_BoundContentConflictFails(t *testing.T) {
	...
	content := &snapshotv1alpha1.PodSnapshotContent{
		ObjectMeta: metav1.ObjectMeta{Name: "podsnapshotcontent-snap-uid"},
		Spec: snapshotv1alpha1.PodSnapshotContentSpec{
			PodSnapshotRef: snapshotv1alpha1.PodSnapshotReference{Namespace: "inference", Name: "podsnapshot-abc123", UID: "other-snap-uid"},
		},
	}
...
func TestSnapshotReconciler_PropagateStatusIsIdempotent(t *testing.T) {
	boundContent := func(reason string) *snapshotv1alpha1.PodSnapshotContent {
		return &snapshotv1alpha1.PodSnapshotContent{
			...
			Status: snapshotv1alpha1.PodSnapshotContentStatus{
				Conditions: []metav1.Condition{{Type: snapshotv1alpha1.PodSnapshotConditionReady, Status: metav1.ConditionTrue, Reason: reason, Message: "done"}},
			},
		}
	}
	...
	t.Run("no-op when already mirrored", func(t *testing.T) {
		...
		meta.SetStatusCondition(&snap.Status.Conditions, metav1.Condition{Type: snapshotv1alpha1.PodSnapshotConditionReady, Status: metav1.ConditionTrue, Reason: "Agent", Message: "done"})
		meta.SetStatusCondition(&snap.Status.Conditions, metav1.Condition{Type: snapshotv1alpha1.PodSnapshotConditionFailed, Status: metav1.ConditionFalse, Reason: "Agent", Message: "done"})
	...
	t.Run("writes when a condition changed", func(t *testing.T) {
		...
		snap := makeSnapshotForReconcile()
		snap.Status.BoundPodSnapshotContentName = ptr.To("podsnapshotcontent-snap-uid")
		meta.SetStatusCondition(&snap.Status.Conditions, metav1.Condition{Type: snapshotv1alpha1.PodSnapshotConditionReady, Status: metav1.ConditionTrue, Reason: "Old", Message: "done"})
		...
		updated := &snapshotv1alpha1.PodSnapshot{}
		require.NoError(t, r.Get(context.Background(), types.NamespacedName{Namespace: "inference", Name: snap.Name}, updated))
		assert.Equal(t, "New", meta.FindStatusCondition(updated.Status.Conditions, snapshotv1alpha1.PodSnapshotConditionReady).Reason)
	})
}
