...
func isPodSnapshotTerminal(snap *snapshotv1alpha1.PodSnapshot) bool {
	return meta.IsStatusConditionTrue(snap.Status.Conditions, snapshotv1alpha1.PodSnapshotConditionFailed)
}
...
func (sr *PodSnapshotReconciler) ensureFinalizer(ctx context.Context, snap *snapshotv1alpha1.PodSnapshot) (bool, error) {
...
func (sr *PodSnapshotReconciler) captureFromSourcePod(ctx context.Context, snap *snapshotv1alpha1.PodSnapshot) (ctrl.Result, error) {
	pod, err := sr.getSourcePod(ctx, snap)
	if err != nil {
		if apierrors.IsNotFound(err) {
			return sr.failPodSnapshot(ctx, snap, "SourcePodNotFound",
...
func (sr *PodSnapshotReconciler) getSourcePod(ctx context.Context, snap *snapshotv1alpha1.PodSnapshot) (*corev1.Pod, error) {
	pod := &corev1.Pod{}
	key := client.ObjectKey{Namespace: snap.Namespace, Name: snap.Spec.Source.PodRef.Name}
...
func validateSourcePod(snap *snapshotv1alpha1.PodSnapshot, pod *corev1.Pod) error {
...
func (sr *PodSnapshotReconciler) ensurePodSnapshotContent(ctx context.Context, snap *snapshotv1alpha1.PodSnapshot, contentName string, pod *corev1.Pod) (*snapshotv1alpha1.PodSnapshotContent, error) {
	content := sr.buildPodSnapshotContent(snap, contentName, pod)
	if err := sr.Create(ctx, content); err != nil {
		if apierrors.IsAlreadyExists(err) {
			existing := &snapshotv1alpha1.PodSnapshotContent{}
...
func (sr *PodSnapshotReconciler) bindContent(ctx context.Context, snap *snapshotv1alpha1.PodSnapshot, contentName string) (ctrl.Result, error) {
...
func (sr *PodSnapshotReconciler) propagateStatus(ctx context.Context, snap *snapshotv1alpha1.PodSnapshot, content *snapshotv1alpha1.PodSnapshotContent) (ctrl.Result, error) {
	var changed bool
	switch {
	case snapshotv1alpha1.IsPodSnapshotContentSucceeded(content):
		cond := meta.FindStatusCondition(content.Status.Conditions, snapshotv1alpha1.PodSnapshotConditionReady)
		changed = sr.markReady(snap, cond.Reason, cond.Message)
	case snapshotv1alpha1.IsPodSnapshotContentFailed(content):
...
func (sr *PodSnapshotReconciler) setCondition(snap *snapshotv1alpha1.PodSnapshot, condType string, status metav1.ConditionStatus, reason, message string) bool {
...
func (sr *PodSnapshotReconciler) markReady(snap *snapshotv1alpha1.PodSnapshot, reason, message string) bool {
	changed := sr.setCondition(snap, snapshotv1alpha1.PodSnapshotConditionReady, metav1.ConditionTrue, reason, message)
	return sr.setCondition(snap, snapshotv1alpha1.PodSnapshotConditionFailed, metav1.ConditionFalse, reason, message) || changed
}
...
func (sr *PodSnapshotReconciler) markFailed(snap *snapshotv1alpha1.PodSnapshot, reason, message string) bool {
	changed := sr.setCondition(snap, snapshotv1alpha1.PodSnapshotConditionFailed, metav1.ConditionTrue, reason, message)
	return sr.setCondition(snap, snapshotv1alpha1.PodSnapshotConditionReady, metav1.ConditionFalse, reason, message) || changed
}
...
func (sr *PodSnapshotReconciler) markPending(snap *snapshotv1alpha1.PodSnapshot, reason, message string) bool {
	return sr.setCondition(snap, snapshotv1alpha1.PodSnapshotConditionReady, metav1.ConditionFalse, reason, message)
}

// failPodSnapshot marks the PodSnapshot Failed terminally (Failed=True, Ready=False) and records
// an event.
func (sr *PodSnapshotReconciler) failPodSnapshot(ctx context.Context, snap *snapshotv1alpha1.PodSnapshot, reason string, cause error) (ctrl.Result, error) {
...
func podSnapshotRefFromContentObj(obj any) (snapshotv1alpha1.PodSnapshotReference, error) {
	if tombstone, isTombstone := obj.(cache.DeletedFinalStateUnknown); isTombstone {
		obj = tombstone.Obj
	}
	content, ok := obj.(*snapshotv1alpha1.PodSnapshotContent)
	if !ok {
		return snapshotv1alpha1.PodSnapshotReference{}, fmt.Errorf("expected *PodSnapshotContent, got %T", obj)
	}
...
func podSnapshotContentName(snap *snapshotv1alpha1.PodSnapshot) string {
